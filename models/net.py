...
class njf_decoder(nn.Module):
    ...
    def predict_map(self, latent_features, source_verts, source_faces, L=None, poisson_solver=None):
        """Predict per-face Jacobians and reconstruct vertices via a Poisson solve.

        Key performance rule:
          - If `poisson_solver` is provided (precomputed by the dataloader), we reuse it directly.
          - Otherwise we fall back to the legacy SourceMesh/MeshProcessor path (which recomputes operators).
        """
        n_faces = source_faces.shape[-2]
        pred_J = self.predict_jacobians(latent_features, n_faces)

        # Fast path: reuse precomputed Poisson solver (no operator recomputation)
        if poisson_solver is not None:
            solver = poisson_solver[0] if isinstance(poisson_solver, (list, tuple)) else poisson_solver
            # Move solver internals to the correct device (in-place)
            solver = solver.to(latent_features.device)
            pred_V = solver.solve_poisson(pred_J)
            return pred_V, pred_J

        # Slow legacy path: builds operators via MeshProcessor (kept for compatibility)
        source = SourceMesh(
            source_v=source_verts,
            source_f=source_faces,
            use_wks=False,
            random_centering=False,
            cpuonly=False,
            L=None,
        )
        source.load(source_v=source_verts, source_f=source_faces, L=L)
